// تسجيل Service Worker للأجهزة والحرار الحديثة
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        navigator.serviceWorker.register('sw.js')
            .then(() => console.log('HIGH TECH PS - SW Registered'))
...
// إدارة تخزين الكاش الحقيقي للبلايستيشن
function initApplicationCache() {
    const appCache = window.applicationCache;

    if (localStorage.getItem('hightech_cached') === 'true') {
        updateProgress(100, 'الكاش مكتمل سابقاً! المشروع يعمل أوفلاين تماماً الآن.');
        return;
    }
    ...
    if (appCache) {
        appCache.addEventListener('progress', (e) => {
            if (e.lengthComputable) {
                ...
                updateProgress(percentage, `جاري تحميل ملفات الكاش: ${percentage}%`);
            }
        });

        appCache.addEventListener('cached', () => {
            localStorage.setItem('hightech_cached', 'true');
            updateProgress(100, 'تم اكتمل التحميل بنجاح (100%)! إفصل الإنترنت الآن.');
        });

        appCache.addEventListener('noupdate', () => {
            localStorage.setItem('hightech_cached', 'true');
            updateProgress(100, 'الكاش محدّث وجاهز للعمل أوفلاين.');
        });

        appCache.addEventListener('error', () => {
            // محاكاة تحذيرية في حالة عدم توفر الأحداث المباشرة
            if (localStorage.getItem('hightech_cached') !== 'true') {
                simulateFallbackCache();
            }
        });
    } else {
        simulateFallbackCache();
...
function simulateFallbackCache() {
    let progress = 0;
    const interval = setInterval(() => {
        ...
        if (progress >= 100) {
            ...
            localStorage.setItem('hightech_cached', 'true');
            updateProgress(100, 'تم اكتمال التحميل بنجاح (100%)! إفصل الإنترنت الآن.');
        } else {
            updateProgress(progress, `جاري تثبيت الكاش: ${progress}%`);
        }
    }, 150);
}
...
function updateProgress(percent, statusMsg) {
    ...
    if (cachePercent) cachePercent.innerText = percent + '%';
    if (cacheStatus) {
        cacheStatus.innerText = statusMsg;
        if (percent === 100) {
...
function launchExploit() {
    ...
    log("جاري تشغيل ثغرة PsFree الأوفلاين...");

    setTimeout(() => {
        log("جاري تجاوز الحماية وتجاوز ASLR عبر WebKit...");
    }, 1000);

    setTimeout(() => {
        log("جاري اختراق النواة وحقن GoldHen...");
    }, 2200);

    setTimeout(() => {
        log("تم تفعيل GoldHen بنجاح!");
        alert("تم تفعيل GoldHen بنجاح بدون الحاجة لأي فلاشة!");
